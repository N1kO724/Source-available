...
description: Transform a user-supplied photograph into an original 3:4 art poster using watercolor, pop-art screenprint, expressive painting, ink-wash, relief print, editorial surrealism, or a controlled hybrid. Use when the user asks to reinterpret a real scene, portrait, object, vehicle, landscape, architecture, or travel photo as creative visual art, an art poster, exhibition poster, editorial poster, or key visual while retaining chosen identity cues.
---
...
- Add concise English poster typography by default unless the user explicitly requests no text. Read and apply [references/typography.md](references/typography.md): use one short all-caps title and, only when useful, one small supporting line derived from the image's subject, mood, motion, place, material, or visual proposition.
...
11. **Typography:** read [references/typography.md](references/typography.md), derive a short English title from the image, place it as part of the composition, and use the locked condensed uppercase editorial form unless the user overrides it.
...
- Preserve requested text exactly. Otherwise derive restrained English copy and apply the locked typography system. Keep wording short enough for reliable rendering; omit a subtitle rather than accept corrupted or decorative filler text.
...
3. **Material, graphic, and type system:** specify physical medium behavior, substrate, palette, selective coverage map, no more than two source-derived graphic devices, exposed negative space, at least one intentional lost-edge transition, and the exact short English title/subtitle with the locked condensed uppercase editorial treatment from [references/typography.md](references/typography.md).
